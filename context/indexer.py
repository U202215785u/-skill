...
def scan_project() -> dict:
    ...
    for filepath in PROJECT_ROOT.rglob("*"):
        if filepath.is_file() and filepath.suffix in CODE_EXTENSIONS:
            ...
            module = filepath.relative_to(PROJECT_ROOT).parts[0] if len(filepath.relative_to(PROJECT_ROOT).parts) > 1 else "root"
    ...
    for file_info in all_files:
        ...
        for pat_ext, pattern in api_patterns:
            if ext == pat_ext:
                try:
                    content = (PROJECT_ROOT / file_info["path"]).read_text(encoding="utf-8", errors="ignore")
